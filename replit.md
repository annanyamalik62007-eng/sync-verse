...
- Pages: `/` Gen Z landing (full-bleed marquee + giant italic SYNCVERSE wordmark + sticker stack + bento "what's hitting" grid + "pick a brain" demo picker grouped by college + leaderboard + manifesto + final CTA wall) and 3-step onboarding form, `/feed` live signals + insights + FOMO, `/matches` (with Message CTA), `/messages` thread list, `/messages/:userId` chat, `/squads` (suggested + active), `/events` per-college events with RSVP + create dialog, `/major` per-major hub, `/zone/:zone` zone deep-dive with **Instagram-like post composer + feed (fire/join reactions)**, `/user/:userId` profile with Instagram-style post tile grid.
- Posts schema: `lib/db/src/schema/posts.ts` — `posts`, `post_reactions` (postId+userId+kind unique), `post_joins` (postId+userId unique). Endpoints under `/api/zones/{zone}/posts`, `/api/users/{userId}/posts`, `POST /api/posts`, `POST /api/posts/{postId}/{react,join}` (toggle). List endpoints return `reactorIds[]` and `joinerIds[]` so the client can compute viewer-state locally without query params (avoids orval naming collision between path-zod and query-type when both are present).
